...
class SiteHelpers
{
	...
	function alert($type = 'info', $caption = '', $title = '') {
    $_this =& get_instance();
    $option = array();
    if( is_string($type )){
      $option['type']     = $type;
      $option['caption']  = $caption;
      $option['title']    = $title?$title:$type;
    }
    if( is_array( $type )){
      $option = array_replace_recursive( array( 
        'type' => 'info' ,
        'title' => (isset($type['title'])&&$type['title'])?$type['title']:$type['type'] ,
        'caption' => '' ,
      ), $type );
    
    }
    return $_this->session->set_flashdata('message',$option);
	}
