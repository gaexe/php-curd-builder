...
class Blogcomment extends SB_Controller 
{

	protected $layout 	= "layouts/main";
	public $module 		= 'blogcomment';
	public $per_page	= '10';
	...
	function __construct() {
		...
		$this->model = $this->blogcommentmodel;
	...
	function index() 
	{
		...
		$sort = (!is_null($this->input->get('sort', true)) ? $this->input->get('sort', true) : 'commentID'); 
		...
		$this->data['content'] = $this->load->view('blogcomment/index',$this->data, true );
	...
	function show( $id = null) 
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('tb_blogcomments'); 
		}
		
		$this->data['id'] = $id;
		$this->data['content'] =  $this->load->view('blogcomment/view', $this->data ,true);	  
		$this->load->view('layouts/main',$this->data);
	}
	...
	function add( $id = null ) 
	{
		...
		$this->data['id'] = $id;
		$this->data['content'] = $this->load->view('blogcomment/form',$this->data, true );		
	  	$this->load->view('layouts/main', $this->data );
	...
	function save() {
		...
		if( $this->form_validation->run() )
		{
			...
			$ID = $this->model->insertRow($data , $this->input->get_post( 'commentID' , true ));
			// Input logs
			...
			{
				redirect( 'blogcomment/add/'.$ID,301);
			} else {
				redirect( 'blogcomment',301);
			}			
