...
class Module extends SB_Controller  {
  ...
  function saveCreate()
    {
      $rules = array(
        array('field'   => 'module_name','label'   => 'Class Controller','rules'   => 'required|is_unique[tb_module.module_name]'),
        array('field'   => 'module_title','label'   => 'Module Name','rules'   => 'required'),
        array('field'   => 'module_db','label'   => 'Module Table','rules'   => 'required'),
    );
    ...
    if( $this->form_validation->run() ){
            ...
            $primary = $this->findPrimarykey( $this->input->post('module_db',true));
            ...
            if($this->input->post('creation') == 'manual')
            {
                if($where =="")
                {
                  SiteHelpers::alert('error','SQL WHERE REQUIRED');
           ...
           // echo '<pre>'; print_r($columns); echo '</pre>';    exit;
            ...
            foreach($columns as $column)
            {
                ...
                {
                    $form_creator = $this->configForm($column->Field,$column->Table,$column->Type,$i);
                    $relation = $this->buildRelation($table ,$column->Field);
                    foreach($relation as $row)
                    {
                        $array = array('external',$row->table,$row->column);
                        $form_creator = $this->configForm($column->Field,$table,'select',$i,$array);
           ...
           // echo '<pre>'; print_r($rowGrid); echo '</pre>';    exit;
            ...
            if(count($row) >= 1)
            {
              ...
              SiteHelpers::alert('error','Failed to create new module !');
  ...
  function saveconfig($id)
  {
    ...
    if ($this->form_validation->run() ) {
      ...
      SiteHelpers::alert('success','Module Info Has Been Save Successfull') ;
  ...
  function saveSql( $id )
  {
    ...
    if(count($row) <= 0){
      ...
      SiteHelpers::alert('error','Can not find module') ;
    ...
    $columns = $this->model->getColoumnInfo($this->db->result_id);
    ...
    SiteHelpers::alert('success','SQL Has Changed Successful.');
  ...
  public function saveTable()
  {
    ...
    if(count($row) <= 0) redirect('sximo/module',301);
      ...
      SiteHelpers::alert('success','Module Table Has Been Save Successfull');
  ...
  function saveForm()
  {
    ...
    if(count($row) <= 0) redirect('sximo/module',301);
        ...
        SiteHelpers::alert('success','Module Forms Has Been Saved Successfull');
  ...
  function saveField()
  {
      ...
      $this->db->update('tb_module',array('module_config' => SiteHelpers::CF_encode_json($new_config)));
        ...
        SiteHelpers::alert('success','Forms Has Changed Successful.');
      redirect('sximo/module/form/'.$row->module_name,301);
    }
    ...
    // echo '<pre>'; print_r($new_config); echo '</pre>';    exit;
      ...
      SiteHelpers::alert('success','Module Table Connection Has Been Save Successfull');
      redirect('sximo/module/table/'.$row->module_name,301);
    }
    ...
    function rebuild( $id = 0)
    {
      ...
      if($this->input->get('rebuild') !='')
      {
        ...
        if($this->input->get('c') =='y'){
          file_put_contents( "application/controllers/{$codes['controller']}.php" , $build_controller) ;

        }
        if($this->input->get('m') =='y'){
          file_put_contents(  "application/models/{$codes['controller']}model.php" , $build_model) ;
        ...
        SiteHelpers::alert('success','Module Created successfull');
        redirect('sximo/module',301);
    }
    ...
    if(count($row) <= 0) redirect('sximo/module',301);
        ...
        $this->db->delete('tb_groups_access');
                ...
                $this->removeDir(  "application/views/{$path}");

        SiteHelpers::alert('success','Module has been removed successfull');
        redirect('sximo/module',301);
            }

        }
    SiteHelpers::alert('error','No Module removed');
    redirect('sximo/module',301);
