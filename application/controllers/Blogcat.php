...
class Blogcat extends SB_Controller 
{

	protected $layout 	= "layouts/main";
	public $module 		= 'blogcat';
	public $per_page	= '10';
	...
	function index() 
	{
		...
		$sort = (!is_null($this->input->get('sort', true)) ? $this->input->get('sort', true) : 'CatID'); 
		...
		$this->data['content'] = $this->load->view('blogcat/index',$this->data, true );
	...
	function show( $id = null) 
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('tb_blogcategories'); 
		}
		
		$this->data['id'] = $id;
		$this->data['content'] =  $this->load->view('blogcat/view', $this->data ,true);	  
		$this->load->view('layouts/main',$this->data);
	}
	...
	function add( $id = null ) 
	{
		...
		$this->data['id'] = $id;
		$this->data['content'] = $this->load->view('blogcat/form',$this->data, true );		
	  	$this->load->view('layouts/main', $this->data );
	...
	function save() {
		...
		if( $this->form_validation->run() )
		{
			...
			$ID = $this->model->insertRow($data , $this->input->get_post( 'CatID' , true ));
			// Input logs
			if( $this->input->get( 'CatID' , true ) =='')
			{
			...
			{
				redirect( 'blogcat/add/'.$ID,301);
			} else {
				redirect( 'blogcat',301);
			}			
